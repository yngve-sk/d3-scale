...
tape("radial(x) maps an empty domain to the range start", function(test) {
  test.equal(scale.scaleRadial().domain([0, 0]).range([1, 2])(0), 1);
  test.equal(scale.scaleRadial().domain([0, 0]).range([2, 1])(1), 2);
  test.end();
});
